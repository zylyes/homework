...
class Ball:
    ...
    def draw(self, surface):
        ...
        rotated_ball = pygame.transform.rotate(pygame.Surface((self.radius*2, self.radius*2)), angle)  # 创建旋转后的小球表面
        rotated_ball.set_colorkey((0, 0, 0))  # 设置透明色
        rotated_ball.fill((0, 0, 0))  # 填充背景色
        pygame.draw.circle(rotated_ball, self.color, (self.radius, self.radius), self.radius)  # 在旋转后的表面上绘制小球
        surface.blit(rotated_ball, (self.x - self.radius, self.y - self.radius))  # 在屏幕上绘制旋转后的小球
    ...
    def move(self, gravity, air_resistance_coeff, friction_coeff, box_bottom):
        ...
        # 空气阻力
        air_resistance_x = air_resistance_coeff * self.velocity[0]**2  # X轴方向的空气阻力
        air_resistance_y = air_resistance_coeff * self.velocity[1]**2  # Y轴方向的空气阻力
        # 对于X轴
        ...
        # 摩擦力 - 当小球在地面上时应用于x方向
        if self.y + self.radius >= box_bottom - 1 and abs(self.velocity[1]) <= min_vertical_speed:
            if abs(self.velocity[0]) > min_horizontal_speed:
                self.velocity[0] -= friction_coeff if self.velocity[0] > 0 else -friction_coeff  # 减小水平速度
            else:
...
class Box:
    ...
    def draw(self, surface):
        pygame.draw.rect(surface, self.color, self.rect, 2)  # 在表面上绘制边界框，线宽为2
...
# 创建球和边界框
ball = Ball(WIDTH // 2, HEIGHT // 2, 13, [40, -55])  # 创建小球对象，初始位置在屏幕中心，半径为10，速度为[40, -55]
box = Box((WIDTH - 600) // 2, (HEIGHT - 400) // 2, 600, 400)  # 创建边界框对象，位于屏幕中心，宽度为600，高度为400
...
while running:
    ...
    # 移动小球
    ball.move(gravity, air_resistance_coeff, friction_coeff, box.rect.bottom)  # 根据物理规则移动小球

    # 边界框碰撞
    if ball.y + ball.radius >= box.rect.bottom:  # 如果小球与底部边界碰撞
        ball.velocity[1] = -ball.velocity[1] * energy_loss_coeff  # 垂直速度反弹并损失能量
        ball.y = box.rect.bottom - ball.radius  # 更新小球位置到边界上
        if abs(ball.velocity[0]) > min_horizontal_speed:  # 如果水平速度大于最小值
            ball.velocity[0] -= friction_coeff if ball.velocity[0] > 0 else -friction_coeff  # 减小水平速度
        else:
            ball.velocity[0] = 0  # 水平速度为0

    # 停止垂直方向的弹跳
    if abs(ball.velocity[1]) < min_vertical_speed and ball.y + ball.radius >= box.rect.bottom - 1:  # 如果垂直速度小于最小值且小球在底部边界上
        ball.velocity[1] = 0  # 垂直速度为0

    # 左右边界碰撞
    if ball.x - ball.radius <= box.rect.left or ball.x + ball.radius >= box.rect.right:  # 如果小球与左右边界碰撞
        ball.velocity[0] = -ball.velocity[0] * energy_loss_coeff  # 水平速度反弹并损失能量
